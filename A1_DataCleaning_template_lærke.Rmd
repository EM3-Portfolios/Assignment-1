...
subset2d <- merge(subset2d_demo, subset2d_LU, by = c("SUBJ", "VISIT"))

# Adding the last small subset to the new subset (overwriting the new subset data frame):
subset2d <- merge(subset2d, subset2d_token, by = c("SUBJ", "VISIT"))
...
subset2f <- select(subset2d, SUBJ, VISIT, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
...
subset2f$Subj <- as.factor(subset2f$SUBJ)
subset2f$Subj <- as.numeric(subset2f$Subj)
subset2f$Subj <- as.character(subset2f$Subj)
