...
# Merging the two of the small subsets together into a new data frame:
merged_df <- merge(subset2d_demo, subset2d_LU, by = c("SUBJ", "VISIT"))

# Adding the last small subset to the new subset (overwriting the new subset data frame):
...
subset2f <- select(merged_df, SUBJ, VISIT, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
...
subset2f <- filter(subset2f, VISIT == 1)

# deleting the visit column
subset2f <- select(subset2f, -VISIT)

# rename clinical measures variables
subset2f <- dplyr::rename(subset2f, ADOS1 = ADOS)
subset2f <- dplyr::rename(subset2f, MullenRaw1 = MullenRaw)
subset2f <- dplyr::rename(subset2f, ExpressiveLangRaw1 = ExpressiveLangRaw)
subset2f <- dplyr::rename(subset2f, Socialization1 = Socialization)

# merging the subset_2f with merged_df / 
merged_df <- merge(merged_df, subset2f, by = "SUBJ")
...
# or:    merged_df$Gender <- chartr("12", "MF", merged_df$Gender)
...
###### Task 2f - alternative ######
```{r}
# I made four ifelse statements (one for each of the clinical columns), that replace the value of the clinical column with NA if the value of the VISIT column in the same row is either 2, 3, 4, 5, or 6. This leaves us with values in the clinical column only from visit 1 of the different participants.
...
#bonus1 %>%
  #group_by(SUBJ) %>%
  #summarise(MLU_across_visits = mean(CHI_MLU))
# But I don't know how to convert it into a df
...
## 3:   ##### So, this doesn't have pipes, but I think it does what it is supposed to do...
