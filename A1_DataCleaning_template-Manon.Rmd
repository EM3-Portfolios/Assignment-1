...
```{r}
# the demo_train dataset has called the first column Child.ID instead of SUBJ as the two other datasets
demo_train <- dplyr::rename(demo_train, SUBJ = Child.ID)

# we need to change demo_train$Visit to demo_train$VISIT
...
```{r}
# LU_train and token_train need their values in column VISIT to only contain the numbers.
...
```{r}
# removing all '.' from values in column 'SUBJ' in all 3 datasets
...
```{r}
# subestting each dataset with desired variables
...
subset_2f <- select(merged_df, SUBJ, VISIT, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)



# have a dataset with 1 row per name from the 1st visitn only and when it merges with the merged_df (after having renamed ADOS to ADOS1 etc of course) the visit 1 values hould be copied to the other 5 visits as well in columns ADOS1 etc. and stay original in the original ADOS column. remember to remove column VISIT in subset_2f before merging with merged_df.

# only including visit 1 in the subset
subset_2f <- filter(subset_2f, VISIT == 1)

# deleting the visit column
subset_2f <- select(subset_2f, -VISIT)

# rename clinical measures variables
subset_2f <- dplyr::rename(subset_2f, ADOS1 = ADOS)
subset_2f <- dplyr::rename(subset_2f, MullenRaw1 = MullenRaw)
subset_2f <- dplyr::rename(subset_2f, ExpressiveLangRaw1 = ExpressiveLangRaw)
subset_2f <- dplyr::rename(subset_2f, Socialization1 = Socialization)

# merging the subset_2f with merged_df / 
merged_df <- merge(merged_df, subset_2f, by = "SUBJ")


#changes to include NAs (post-answer from Byurakn)
...
# Change gender markes from 1 and 2 to M and F
merged_df$Gender <- chartr("12", "MF", merged_df$Gender)

# Change the diagnosis markes from A and B to ASD and TD
# this function takes (old value, new value, df$column) and is easier to read than the chartr() function
